...
<%
	...
	ArrayList<Emp> list = new ArrayList<>();  // 개발자로 매우 중요함*** 데이터를 가공하는 파트. 
	while(rs.next()) {
		Emp e = new Emp(); // 행의 갯수만큼 emp가 필요함(결과행의 수만큼 emp객체 필요)
	...
	<table border="1">
		...
		%>
			<tr>
				...
				<td><%=e.createdate%></td>
				<td><%=e.updatedate%></td>
				<td><a href="<%=request.getContextPath()%>/updateEmpForm.jsp?empNo=<%=e.empNo%>">수정</a> </td>
				<td>
				<a href="<%=request.getContextPath()%>/deleteEmpAction.jsp?empNo=<%=e.empNo%>">삭제</a></td>
